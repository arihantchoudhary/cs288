...
class Tokenizer:
    ...
    def tokenize(self, text: str) -> List[int]:
        ...
        return [self.token2id[w] for w in words if w in self.token2id]
...
class BOWDataset(Dataset):
    ...
    def __getitem__(
        ...
        label_t = torch.tensor(
            self.label2id.get(dp.label, 0), dtype=torch.int64
        )
...
class Trainer:
    ...
    def evaluate(self, data: BOWDataset) -> float:
        ...
        targets = [data.label2id[data.data[i].label] for i in range(len(data))]
